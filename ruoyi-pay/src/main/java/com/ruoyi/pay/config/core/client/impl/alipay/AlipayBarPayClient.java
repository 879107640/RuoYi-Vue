package com.ruoyi.pay.config.core.client.impl.alipay;
...
import static com.ruoyi.pay.config.core.client.impl.alipay.AlipayPayClientConfig.MODE_CERTIFICATE;
...
public class AlipayBarPayClient extends AbstractAlipayPayClient {
  ...
  public PayOrderRespDTO doUnifiedOrder(PayOrderUnifiedReqDTO reqDTO) throws AlipayApiException {
    ...
    if ("10000".equals(response.getCode())) { // 免密支付
      ...
      PayOrderRespDTO payOrderRespDTO = PayOrderRespDTO.successOf(response.getTradeNo(), response.getBuyerUserId(), successTime, response.getOutTradeNo(), response);
    ...
    return PayOrderRespDTO.waitingOf(displayMode, "", reqDTO.getOutTradeNo(), response);
  }
