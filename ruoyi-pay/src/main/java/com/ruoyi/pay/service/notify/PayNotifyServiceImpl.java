...
import com.ruoyi.pay.mapper.notify.PayNotifyLogMapper;
import com.ruoyi.pay.mapper.notify.PayNotifyTaskMapper;
import com.ruoyi.pay.redis.notify.PayNotifyLockRedisDAO;
...
import com.ruoyi.pay.service.dto.notify.dto.PayTransferNotifyReqDTO;
...
import static com.ruoyi.pay.framework.job.config.PayJobConfiguration.NOTIFY_THREAD_POOL_TASK_EXECUTOR;
...
public class PayNotifyServiceImpl implements PayNotifyService {
  ...
  public void executeNotify0(PayNotifyTaskDO task) {
    ...
    PayNotifyLogDO notifyLog = PayNotifyLogDO.builder().taskId(task.getId())
   ...
   * @return HTTP 响应
   */
  private AjaxResult executeNotifyInvoke(PayNotifyTaskDO task) {
    // 拼接 body 参数
    Object request;
    if (Objects.equals(task.getType(), PayNotifyTypeEnum.ORDER.getType())) {
    ...
    try (HttpResponse response = HttpUtil.createPost(task.getNotifyUrl())
      ...
      // 解析结果
      return JsonUtils.parseObject(response.body(), AjaxResult.class);
    }
  }
   ...
   */
  @VisibleForTesting
  Integer processNotifyResult(PayNotifyTaskDO task, AjaxResult invokeResult, Throwable invokeException) {
