package com.ruoyi.pay.config.core.client.impl.weixin;
...
import com.ruoyi.pay.config.core.enums.order.PayOrderDisplayModeEnum;
...
import static com.ruoyi.common.utils.json.JsonUtils.toJsonString;
...
public class WxBarPayClient extends AbstractWxPayClient {
    ...
    protected PayOrderRespDTO doUnifiedOrderV2(PayOrderUnifiedReqDTO reqDTO) throws WxPayException {
        ...
        for (int i = 1; i < Byte.MAX_VALUE; i++) {
            try {
                ...
                PayOrderRespDTO payOrderRespDTO = PayOrderRespDTO.successOf(response.getTransactionId(), response.getOpenid(), parseDateV2(response.getTimeEnd()),
                    response.getOutTradeNo(), response);
                payOrderRespDTO.setDisplayMode(PayOrderDisplayModeEnum.BAR_CODE.getMode());
    ...
    static String getAuthCode(PayOrderUnifiedReqDTO reqDTO) throws WxPayException {
        String authCode = MapUtil.getStr(reqDTO.getChannelExtras(), "authCode");
        if (StrUtil.isEmpty(authCode)) {
            throw new WxPayException("支付请求的 authCode 不能为空！");
        }
        return authCode;
    }
